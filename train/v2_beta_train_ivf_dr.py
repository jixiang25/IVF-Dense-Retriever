...
from model.v2_ivf_dense_retriever import IvfDenseRetriever, ClusterMarginLoss
...
def train_epoch(args, model, ranking_criterion, clustering_criterion, optimizer, scheduler, 
    train_dataloader, writer, update_steps, train_loss, similarity_func):
    model.train()
    device = model.device
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        query_centroid_ids = batch["top_centroid_ids"].to(device)
        ...
        loss = clustering_criterion(clustering_score, query_centroid_ids)
        ...
        if (step + 1) % args.gradient_accumulate_steps == 0:
            ...
            update_steps += 1
            description = "loss:{:.6f}".format(train_loss / update_steps)
...
def train(args):
    ...
    clustering_criterion = ClusterMarginLoss(args.nearest_centroid_num, args.cluster_num)
