...
class IvfDenseRetriever(BertPreTrainedModel):
    ...
    def forward(self, input_ids, attention_mask, return_top_centroid):
        ...
        # return centroid for query encoder
        if return_top_centroid:
            query_centroid_scores = l2_distance(text_embedding, self.centroid_embeddings)
            _, top_centroid_ids = query_centroid_scores.topk(k=self.nearest_centroid_num, dim=-1)
            return text_embedding, top_centroid_ids
        else:
